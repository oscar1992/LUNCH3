...
class VistaEntregado: UIView {
    ...
    func botonEditar(){
        ...
        botEditar.addTarget(self, action: #selector(VistaEntregado.pasaTipo), forControlEvents: .TouchDown);
    }
    
    func pasaTipo(){
        let cargaT = CargaTipos();
        cargaT.cargaTipos(self.pedido);
        DatosB.cont.historial.performSegueWithIdentifier("Tipos", sender: nil);
    }
